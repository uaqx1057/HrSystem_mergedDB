...
class HrComplianceController extends AccountBaseController
{
    ...
    public function index(Request $request){ $this->pageTitle = 'HR compliance'; $this->authorizeHr(); $company=user()->company_id; $this->employees=User::allEmployees(null,false,'all',$company)->load('employeeDetail','visa'); $this->reviews=HrProbationReview::with('employee')->where('company_id',$company)->latest()->get(); $this->certifications=HrCertification::with('employee')->where('company_id',$company)->latest()->get(); $this->requiredCertificationGaps=HrCertificationRule::missingForEmployees($this->employees,HrCertificationRule::where('company_id',$company)->where('is_active',true)->get(),$this->certifications); $this->cases=HrRestrictedCase::where('company_id',$company)->latest()->get(); $this->expiryBand=$request->input('expiry_band','all'); $this->expiryType=$request->input('expiry_type','all'); $this->expiryItems=$this->expiryItems($this->employees,$company)->filter(fn($item)=>($this->expiryBand==='all'||$item['band']===$this->expiryBand)&&($this->expiryType==='all'||$item['type']===$this->expiryType))->values(); return view('hr-compliance.index',$this->data); }

    private function expiryItems($employees, int $company)
    {
        return HrDocumentExpiry::forEmployees($employees, $company);
    }
