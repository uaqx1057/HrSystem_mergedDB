...
<div class="row mt-4">
    ...
    @if($company->headers)
        <div class="col-xl-12 mt-4">
            <x-cards.data :title="__('superadmin.registeredHeaders')">
                <pre>{!! $company->headers?stripslashes($company->headers):'-' !!}</pre>
                <pre>{{__('superadmin.registeredIp')}} <strong>{{$company->register_ip??'-'}}</strong></pre>
            </x-cards.data>
        </div>
    @endif
</div>
