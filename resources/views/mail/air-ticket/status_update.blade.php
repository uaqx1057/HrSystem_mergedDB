...
<head>
    ...
    <style>
        ...
        .status-badge { padding: 4px 12px; border-radius: 20px; color: white; font-size: 14px; background-color: {{ $ticket->status == 'approved' ? '#10b981' : '#ef4444' }}; }
        .reason-box { margin-top: 15px; padding: 15px; background-color: #fbe8ff; border-left: 4px solid #722C81; font-style: italic; }
...
<body>
    <div class="wrapper">
        <div class="email-content">
            <div class="header"><h1>Air Ticket Request {{ ucfirst($ticket->status) }}</h1></div>
            <div class="body">
                <p>Hello {{ $ticket->employee->name }},</p>
                <p>There has been an update regarding your air ticket request.</p>
                <div class="details-box">
                    ...
                    @if ($ticket->status == 'approved' && $ticket->approve_reason)
                        <div class="reason-box"><strong>Note:</strong> {{ $ticket->approve_reason }}</div>
                    @elseif($ticket->status == 'rejected' && $ticket->reject_reason)
                        <div class="reason-box"><strong>Reason:</strong> {{ $ticket->reject_reason }}</div>
                    @endif
                </div>
