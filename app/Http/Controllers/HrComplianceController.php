...
class HrComplianceController extends AccountBaseController
{
    ...
    public function index(){ $this->pageTitle = 'HR compliance'; $this->authorizeHr(); $company=user()->company_id; $this->employees=User::allEmployees(null,false,'all',$company)->load('employeeDetail'); $this->reviews=HrProbationReview::with('employee')->where('company_id',$company)->latest()->get(); $this->certifications=HrCertification::with('employee')->where('company_id',$company)->latest()->get(); $this->requiredCertificationGaps=HrCertificationRule::missingForEmployees($this->employees,HrCertificationRule::where('company_id',$company)->where('is_active',true)->get(),$this->certifications); $this->cases=HrRestrictedCase::where('company_id',$company)->latest()->get(); return view('hr-compliance.index',$this->data); }
