...
@push('scripts')
    ...
    <script>
        // ✅ Fix 7: Changed all 'insurances-table' references to 'air-tickets-table'
        $('#air-tickets-table').on('preXhr.dt', function (e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["air-tickets-table"].draw(false);
        }
        ...
        const applyQuickAction = () => {
            // ✅ Fix 8: Changed 'insurances-table' to 'air-tickets-table' for checkbox selection
            const rowdIds = $("#air-tickets-table input:checkbox:checked").map(function () {
                return $(this).val();
            }).get();

            const url = "{{ route('air-tickets.apply_quick_action') }}?row_ids=" + rowdIds;
